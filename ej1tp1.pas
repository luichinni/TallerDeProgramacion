...
procedure AlmacenarInformacion (var v: vector; var dimL: rango3);
	//PROCESO INTERNO ALMACENAR INFO
...
//----------------------------------------------------------------------
// ORDENAR VECTOR POR CODIGO DE VENTA
...
Begin
	...
	then begin
		...
		for i:=posInferior to dimL do begin
			if((i+salto)<=dimL)then
				v[i]:=v[i+salto];
		end;
		dimL:=dimL-salto;
	end;
...
procedure GenerarLista (v: vector; dimL: rango3; var L: lista);
  ...
  var
	aux:lista;
  begin
    new(aux); aux^.sig:=L; aux^.dato:=elem;
    L:=aux;
  end;
...
var
    ...
    dimL: rango3;
    L:lista;
    valorSuperior,valorInferior: rango1;
